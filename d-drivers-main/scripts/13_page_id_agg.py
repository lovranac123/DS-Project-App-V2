...
df = pd.read_csv('C:/Data Science Project/data/data_aggr.csv', parse_dates=['date', 'publish_date'])
...
def aggregate_strings(url_column):
    ...
    url_column = url_column.fillna('').astype(str)
...
df['publish_date_min'] = pd.to_datetime(df['publish_date_min'], errors='coerce')

# Check for and handle any NaT values
if df['publish_date_min'].isnull().any():
...
df.to_csv('C:/Data Science Project/data/data_aggr_page_id.csv', index=False)

print('Aggregated data saved as "C:/Data Science Project/data/data_aggr_page_id.csv" ')
print()
